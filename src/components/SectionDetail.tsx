...
import { ArrowLeft, Feather, BookOpen, Camera, Video, FileText, Heart, Globe, User, MessageSquare } from 'lucide-react';
...
const SectionDetail: React.FC<SectionDetailProps> = ({ currentLang }) => {
  ...
  const sectionsData: { [key: string]: any } = {
    ...
    biography: {
      ...
      titleKg: 'Өмүр баяны',
      icon: <User size={80} />,
      descriptionRu: 'Жизненный путь и творческая биография Амана Токтогулова.',
      descriptionKg: 'Аман Токтогулдун өмүр жолу жана чыгармачылык биографиясы.',
      contentRu: 'Аман Токтогулов - выдающийся кыргызский поэт и писатель. Его творческий путь начался в раннем возрасте и продолжался на протяжении многих лет, оставив глубокий след в кыргызской литературе.',
      contentKg: 'Аман Токтогулов - көрүнүктүү кыргыз акыны жана жазуучусу. Анын чыгармачылык жолу эрте куракта башталып, көп жылдар бою улануу менен кыргыз адабиятында терең из калтырган.'
    },
    feedback: {
      titleRu: 'Отзывы и предложения',
      titleKg: 'Сайт боюнча ой-пикирлер жана сунуштар',
      icon: <MessageSquare size={80} />,
      descriptionRu: 'Ваши мысли о сайте, отзывы и предложения по улучшению.',
      descriptionKg: 'Сайт жөнүндө ой-пикирлериңиз, пикирлер жана жакшыртуу боюнча сунуштар.',
      contentRu: 'Мы будем рады услышать ваше мнение о сайте. Ваши отзывы и предложения помогут нам сделать этот ресурс лучше и удобнее для всех посетителей.',
      contentKg: 'Биз сайт жөнүндө сиздин оюңузду угууга кубанабыз. Сиздин пикирлериңиз жана сунуштарыңыз бул ресурсту бардык келүүчүлөр үчүн жакшыраак жана ыңгайлуураак кылууга жардам берет.'
    }
  };
